...
const mapProduct = (product: any, latitude: number, longitude: number) => {
  ...
  return {
    ...
    pickupWindows: (() => {
      ...
      if (sellerDays) {
        const days = sellerDays
          .split(',')
          .map((d: string) => d.trim())
          .join(', ');
        return [
          { days, startTime: sellerStart, endTime: sellerEnd, formatted: `${days} ${sellerStart} - ${sellerEnd}` },
        ];
      }
      return [];
    })(),
...
export const resolvers = {
  Query: {
    ...
    nearbyProducts: async (_parent: any, args: { location: any; category?: string }, context: any) => {
      ...
      return products
        .map((product: any) => mapProduct(product, latitude, longitude))
        .filter((p: any) => {
    ...
    availableSampleSellers: async (_parent: any, args: { orderId: string }, context: any) => {
      ...
      const sellers = samples.map((sample: any) => {
        ...
        return {
          ...
          distanceMiles: parseFloat(distance.toFixed(1)),
          disclaimer:
            'This is a complimentary sample. Loopa does not take responsibility for product quality, ingredients, allergens, or safety. Please review details carefully before claiming.',
          pickupWindows,
        };
      });
    ...
    nearbyFreeSampleProducts: async (_parent: any, args: { location: any }, context: any) => {
      ...
      const eligibleSellerIds = samplingSellerIds.filter((id: string) => !excludedSellerIds.has(id));
      ...
      for (const product of products) {
        ...
        const overallRating =
          reviewCount > 0
            ? parseFloat(
                (allReviews.reduce((sum: number, r: any) => sum + (r.overallRating || 0), 0) / reviewCount).toFixed(1),
              )
            : null;

        // Collect unique tags: product tags + seller categories
        const tags = Array.from(new Set([...(product.tags || []), ...(product.seller.categories || [])]));
  ...
  Mutation: {
    createCategory: async (
      _parent: any,
      args: { label: string; icon: string; isActive?: boolean; count?: number },
      context: any,
    ) => {
    ...
    createOrder: async (
      _parent: any,
      args: { input: { customerId: string; items: Array<{ productId: string; quantity: number }> } },
      context: any,
    ) => {
      ...
      return {
        ...
        freeSampleOffer: {
          ...
          description:
            'Because you supported a local maker today, someone else in the neighborhood wants to share a little goodness with you.',
        },
      };
    },
    claimSample: async (
      _parent: any,
      args: { input: { orderId: string; sampleId: string; sellerId: string; pickupWindowId: string } },
      context: any,
    ) => {
