  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on(
      ...
      async ({
        ...
        const unreadUpdate =
          chat.participant1Id === receiverId ? { unreadCount1: { increment: 1 } } : { unreadCount2: { increment: 1 } };
    ...
    socket.on('message-read', async ({ chatId }: { chatId: string }) => {
      ...
      const unreadReset = chat.participant1Id === userId ? { unreadCount1: 0 } : { unreadCount2: 0 };
